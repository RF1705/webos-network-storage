...
mkdir -p "$PROFILE_DIR" "$CREDENTIAL_DIR" "$STATE_DIR/bin" "$TMP_DIR/mounts" "$TMP_DIR/jails" "$TEST_BIN"
REAL_STAT=$(command -v stat)
for command_name in id sed wc tr mkdir chmod ln awk touch rm; do ln -s "$(command -v "$command_name")" "$TEST_BIN/$command_name"; done
cat > "$TEST_BIN/stat" <<EOF
#!/bin/sh
if [ "\${1-}" = -c ] && [ "\${2-}" = %d ]; then
	case \${3-} in
		"$TMP_DIR/mounts/fritz-games") if [ -f "$TMP_DIR/root-mounted" ]; then echo 9001; exit 0; fi ;;
		"$TMP_DIR/jails/org.scummvm.scummvm$TMP_DIR/mounts/fritz-games") if [ -f "$TMP_DIR/jail-mounted" ] && [ ! -f "$TMP_DIR/jail-recreated" ]; then echo 9001; exit 0; fi ;;
	esac
fi
exec "$REAL_STAT" "\$@"
EOF
...
run_helper() {
	WNS_STATE_DIR=$STATE_DIR WNS_MOUNT_ROOT=$TMP_DIR/mounts WNS_JAIL_ROOT=$TMP_DIR/jails WNS_TEST_MODE=true WNS_ALLOW_UNPRIVILEGED=true WNS_SKIP_PERMISSION_CHECK=true "$HELPER" "$@"
}
expect_failure() { if "$@" >"$TMP_DIR/output" 2>&1; then echo "Expected command to fail: $*" >&2; exit 1; fi; }
...
sed 's#REMOTE_PATH=/games#REMOTE_PATH=/exports/../etc#' "$PROFILE_DIR/games.conf" > "$PROFILE_DIR/traversal.conf"
...
sed 's#REMOTE_PATH=/games#REMOTE_PATH=$(printf injected)#' "$PROFILE_DIR/games.conf" > "$PROFILE_DIR/injection.conf"
...
#!/bin/sh
printf '%s\n' "\$*" >> "$TMP_DIR/mount-args"
if [ "\${1-}" = --bind ]; then touch "$TMP_DIR/jail-mounted"; rm -f "$TMP_DIR/jail-recreated"; fi
exit 0
EOF
...
exit 0
EOF
chmod 0755 "$TEST_BIN/fusermount" "$TEST_BIN/mount" "$TEST_BIN/umount" "$STATE_DIR/bin/rclone-smb"

PATH=$TEST_BIN WNS_RCLONE_BIN=$STATE_DIR/bin/rclone-smb WNS_MOUNT_BIN=$TEST_BIN/mount WNS_UMOUNT_BIN=$TEST_BIN/umount run_helper mount fritz | grep -q "is mounted"
